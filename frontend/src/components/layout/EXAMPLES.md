...
1. **页面标题**：设置 route meta，可在页头自动显示页面标题。
2. **加载状态**：使用 `appStore.setLoading(true/false)` 显示全局加载指示器。
3. **Toast 通知**：使用 `appStore.showSuccess()`、`appStore.showError()` 等。
4. **认证**：所有已认证页面都应使用 `AppLayout`。
5. **认证页面**：Login 和 Register 页面应使用 `AuthLayout`。
6. **侧边栏状态**：侧边栏状态会跨导航保留。
7. **移动优先**：所有示例默认使用 Tailwind 的移动优先方式实现响应式。
